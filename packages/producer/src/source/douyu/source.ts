/**
 * DouyuSource — 斗鱼直播 as a source adapter.
 *
 * Wraps the `DouyuSite` live site (JS + CryptoJS signing via `host.js`) behind
 * the `SourceAdapter` seam: a subscription carries a roomId in `config`, `fetch`
 * emits a single `FeedLive`, `resolveLivePlay` lazily resolves playable URLs,
 * and `listRecommendRooms` powers live discovery UIs.
 */
...
import { DouyuSite } from "./site.ts"
import { toFeedLive } from "../../utils/to-feed-live.ts"

export class DouyuSource extends BaseSource<Subscription> {
  readonly sourceId = "douyu" as const
  readonly builtinSubscriptions = [
    { id: "live-douyu", title: "斗鱼直播示例", tag: "API · 直播", config: { roomId: "3" } },
  ] as const
  readonly meta = {
    name: "斗鱼直播",
    description: "房间号订阅",
  ...
  private readonly site: DouyuSite

  constructor(host: ProducerHost) {
    super()
    this.site = new DouyuSite(host)
  }
  ...
  createSubscription(
    ...
    return { ...base, sourceId: "douyu", config: { roomId: String(config.roomId ?? "") } }
  }

  async fetch(subscription: Subscription, host: ProducerHost): Promise<FeedLive[]> {
    const roomId = String(subscription.config.roomId ?? "")
    if (!roomId) throw new Error("douyu: roomId is required")
    const detail = await this.site.getRoomDetail(roomId)
    return [toFeedLive({ platform: "douyu", roomId, detail, now: host.now() })]
  }
