...
> 附带红利:三大探针立起后,**清退大部分数据路径 debug log**(见 §七)。
>
> ✅ **落地状态(2026-08-28)**:八命令(channels/fetch/item/play/dm/hot/align/env/refresh)
> 全部可用(`bun run rss <cmd>`),bili video/live、douyu、youtube 实测通过。待办:
> M3 `--log` 域开关、M4 §七 log 清退。
...
- ⛔ **浏览器模拟渠道暂不考虑**:`weibo:user`、`xhs:*` 及扫码登录(`login`)整条线
  ...
  自动降级),CLI 其余部分零改动。`crawler/src/example/browser-sim.ts` 是唯一保留的
  example 脚本(CLI 排除该类渠道;**2026-09-11 起零依赖**:bun 内置 WebSocket 直连 CDP)。
...
apps/cli/
  ...
  src/
    ...
    store/sqlite-storage.ts # StorageBackend 实现(bun:sqlite,§六)
    commands/*.ts       # 命令实现(§四)
...
| localStorage | storage backend(**已切 bun:sqlite**,§六) | ✅ |
...
rss refresh [sub]                   # core createDataLayer 编排冒烟(SQLite 持久化)
...
散装 example 已全部收编转正并于 2026-08-31 清退;**唯一保留** `crawler/src/example/browser-sim.ts`
(浏览器模拟,CLI 排除该类渠道)。新增调试能力只写一处(CLI),不再新起 example。

## 六、存储:bun:sqlite(CLI) / tauri-plugin-sql(desktop)
...
- 两端都**不用插件/库各自的 migration 机制**(防与对方各写各的 SQL 漂移);desktop 用
  官方 tauri-plugin-sql(sqlx 底层),CLI 用内置 `bun:sqlite`;StorageBackend 语义保持
  (KV string),三 repo 与 commands 全身而退。desktop 首次访问自动从 localStorage 搬迁
  subscriptions/reading/settings 三 key 并删 local 防双源漂移。选型/迁移/验证状态见
  `docs/sqlite-storage-research.md`。
- WAL:两端各持独立库文件、单进程访问,暂不开;sidecar 互通(同库文件)时再开。

## 七、log 清退联动(CLI 的最大一次性红利,**未执行待办**)
...
具体动作:M1-M2 之后分批执行(现 `resolve/src/log.ts` 仍引用本表,勿删本节)——
...
- 现状:`--log <域>` 已可用;`RSS_LOG=1` 全开(CLI 无 localStorage,垫了默认 `log="0"` 的 shim)
...
1. ✅ **M1 骨架 + 三大探针**(2026-08-28):`channels`/`fetch`/`item` 打通 workspace 引用链
2. ✅ **M2 核心闭环**(2026-08-28):`dm` + `align` + `env` + settings 持久化 +
   `refresh` 冒烟(`play --open` 顺带完成)
3. ✅ **M2.5 example 清退 + hot 命令**(2026-08-31):见 §五;新增 `rss hot <词>`
4. **M3 打磨**:`--log` 域开关全面接 `@tauri-playground/log`、表格渲染打磨
5. **M4 log 清退**:按 §七 分批清退数据路径 log + 精简域(探针已在岗,顺序前提满足)
...
- **未来 sidecar 地基**(desktop 爬虫服务化,2026-08-25 决策):node-host 基建外套
  HTTP 层(hono/fastify)即是 sidecar,CLI 先行是该里程碑的第一块
