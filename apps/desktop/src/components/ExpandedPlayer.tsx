...
export function ExpandedPlayer({
  ...
  item: MediaItem
  /** 懒解析 video 可播流(绑定 item.subscriptionId;返回流 + 弹幕能力)。 */
  resolvePlay: (itemId: string) => Promise<ResolvePlayback>
  /** 懒解析 live 可播流(返回流 + 弹幕能力)。 */
  resolveLivePlay: (roomId: string) => Promise<ResolvePlayback>
  onClose: () => void
}) {
  ...
  // resolve 一次拿齐流 + 弹幕能力(core 已探测 DanmakuPlayable);弹幕生命周期
  // (订阅/退订)收敛在 player 的 DanmakuLayer,desktop 零弹幕逻辑。
    ...
    >
      <div className="relative w-[min(72rem,90vw)]">
        ...
        {/* 大播放器:key 绑 item.id,打开即全新挂载 → autoResolve 起播。
            外层 relative aspect-video 定 16:9(父 w-[min(72rem,90vw)] 用 vw 首帧即确定,
            解析中/错误等中间状态也有稳定高度);VideoShell 统一自撑 16:9 填满本层。 */}
        <div key={item.id} className="relative flex aspect-video items-center justify-center overflow-hidden rounded-lg bg-black shadow-2xl">
          <PlayableMedia
